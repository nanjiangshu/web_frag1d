#!/usr/bin/perl -w
# Filename:   check_frag1d.bioshu.se.pl

# Description: check whether frag1d.bioshu.se is accessable and also check the status
...
my $target_qd_script_name = "qd_frag1d_fe.py";
...
# second: check if qd_frag1d_fe.pl running at pcons1.scilifelab.se frontend
    my $num_running=`curl $url/cgi-bin/check_qd_frag1d_fe.cgi 2> /dev/null | html2text | grep  "$target_qd_script_name" | wc -l`;
    chomp($num_running);

    if ($num_running < 1){
        $output=`curl $url/cgi-bin/restart_qd_frag1d_fe.cgi 2>&1 | html2text`;
