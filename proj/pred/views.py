...
path_log_divided = "%s/static/log/divided"%(SITE_ROOT)
...
def index(request):#{{{
    ...
    if not os.path.exists(path_log):
        os.mkdir(path_log, 0755)
    if not os.path.exists(path_log_divided):
        os.mkdir(path_log_divided, 0755)
...
# get seqid_index_map
    if os.path.exists(finished_seq_file):
        resultdict['index_table_header'] = ["No.", "Length", "%H (sec)", 
                "%S (sec)", "%H (S3)", "%S (S3)", "RunTime(s)", "SequenceName", "Source" ]
...
#}}}
    ...
    predfile = "%s/query.predfrag1d"%(topfolder_seq0)
    nicehtmlfile = "%s/query.predfrag1d.html"%(topfolder_seq0)
    if os.path.exists(predfile):
        if not os.path.exists(nicehtmlfile):
            CreateNiceHTMLFile(predfile, nicehtmlfile)
        resultdict['nicehtmlfile'] =  "%s/%s/%s/%s/%s"%(
                "result", jobid, jobid, subdirname,
                os.path.basename(nicehtmlfile))
    else:
        resultdict['nicehtmlfile'] = ""
